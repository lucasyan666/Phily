...
final class DefaultCamera: NSObject, Camera {
  ...
  private func handleSampleBufferStreaming(_ sampleBuffer: CMSampleBuffer) {
    ...
    // PHILY PATCH: guard against a non-finite exposure duration. During camera
    // switchover (virtual triple-camera zoom) or exposure-mode transitions,
    // `exposureDuration` can be invalid/indefinite, making `.seconds` NaN or ∞.
    // `Int(NaN)` traps with a fatal error, crashing the capture-session queue.
    let exposureSeconds = captureDevice.exposureDuration.seconds
    let sensorExposureTime = exposureSeconds.isFinite
      ? Int(exposureSeconds * 1_000_000_000)
      : 0
